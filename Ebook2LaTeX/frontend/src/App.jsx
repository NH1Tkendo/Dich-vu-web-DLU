...
import { uploadDocument, processDocument, batchUpdateFormulas } from './services/api';

// ── Inline: FormulaEditorItem ──────────────────────────────────────
function FormulaEditorItem({ formula, index, onChange }) {
  const { latex, isDirty, setFromLatex, setFromMathLive } =
    useFormulaSync(formula.raw_latex);
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-3">
        <h3 className="text-sm font-semibold text-gray-300">Formula #{index + 1}</h3>
        <div className="flex items-center gap-2">
          {isDirty && (
            <span className="text-xs text-warning animate-pulse-slow">● unsaved</span>
          )}
...
function App() {
  ...
  const handleUpload = async (file) => {
    ...
    try {
      ...
      alert('Đã xảy ra lỗi trong quá trình xử lý file PDF. Vui lòng xem console.');
    } finally {
    ...
    <div className="min-h-dvh bg-surface-900 bg-grid-pattern bg-grid font-sans text-white">
      ...
      <header className="sticky top-0 z-30 border-b border-surface-700 bg-surface-900/80 backdrop-blur-md">
        <div className="mx-auto flex max-w-5xl items-center justify-between px-6 py-4">
          ...
          <span className="rounded-full bg-primary-500/20 px-3 py-1 text-xs font-medium text-primary-300 ring-1 ring-primary-500/30">
            Live AI Processing
          </span>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-5xl px-6 py-10" id="main-content">
        <section aria-labelledby="upload-heading" className="mb-10">
          <h2 id="upload-heading" className="mb-1 text-2xl font-bold tracking-tight">
            Upload your PDF
          </h2>
          <p className="mb-6 text-sm text-gray-500">
            We'll extract every mathematical formula and let you review each one.
          </p>
          ...
          {isProcessing && (
            <div className="mt-4 flex items-center gap-3 text-primary-300 animate-pulse">
              ...
              <span>Đang phân tích AI (OCR)... Vui lòng chờ.</span>
            </div>
          )}
          
          {(!isProcessing && uploadedFile) && (
            <p className="mt-3 text-sm text-gray-500">
              Đã phân tích: <span className="text-primary-300 font-medium">{uploadedFile.name}</span>
            </p>
          )}
        </section>

        {formulas.length > 0 && (
          <section aria-labelledby="formulas-heading" className="animate-fade-in">
            <div className="mb-6 flex items-center justify-between gap-4">
              <div>
                <h2 id="formulas-heading" className="text-2xl font-bold tracking-tight">
                  Extracted Formulas
                </h2>
                <p className="mt-1 text-sm text-gray-500">
                  {formulas.length} formulas found · Edit directly in the visual editor or raw LaTeX
                </p>
              </div>
              <button
                id="submit-btn"
      ...
      <footer className="mt-20 border-t border-surface-700 py-6 text-center text-xs text-gray-600">
        Ebook2LaTeX · Powered by FastAPI + PyMuPDF + pix2tex · React
      </footer>
    </div>
  );
}
